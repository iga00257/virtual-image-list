...
export default function Home({ breeds }: HomeProps) {
  ...
  const filteredBreeds = breeds.filter((breed) => breed.name.toLowerCase().includes(searchTerm.toLowerCase()))
          ...
          <div className='max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4'>
            <div className='flex items-center justify-between'>
              <div className='flex items-center'>
                <h1 className='sm:block text-xl font-semibold text-gray-900'>可愛狗狗圖庫</h1>
              </div>
            </div>
          </div>
        </div>
